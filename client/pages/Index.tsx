...
export default function Index() {
  ...
  const filtered = listings.filter((l) => {
    const inQuery = `${l.title} ${l.address} ${l.locality}`
      .toLowerCase()
  ...
  const confirmPayment = () => {
    ...
    toast({
      ...
      description: `Paid ₹${payment.amount.toLocaleString()} for ${openPayFor.title}`,
    });
  };
    ...
    <div>
        ...
        <div className="container relative z-10 py-20 md:py-28 text-white">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 text-white/90 md:text-lg">
              Browse verified paying guest stays across Dehradun, Rishikesh,
              Nainital, and more. Search, filter, and pay rent online—simple.
            </p>
          </div>
          <div
            id="search"
            className="mt-8 rounded-xl bg-background/90 backdrop-blur border p-4 shadow-lg"
          >
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <Select
                value={loc}
                onValueChange={(v) => setLoc(v === "__all__" ? "" : v)}
              >
                <SelectTrigger>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {(
                ...
                <button
                  ...
                  className={
                    "px-3 py-1 rounded-full text-xs border transition-colors flex items-center gap-2 " +
              ...
              {amenities.length > 0 && (
                <Button variant="ghost" onClick={() => setAmenities([])}>
                  Clear amenities
                </Button>
              )}
            </div>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Dialog>
                    <DialogTrigger asChild>
                      <Button size="sm" onClick={() => startPay(l)}>
                        Pay Rent
                      </Button>
                    </DialogTrigger>
                  </Dialog>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {l.amenities.map((a) => (
                    <span
                      key={a}
                      className="px-2 py-1 rounded-full bg-accent text-xs inline-flex items-center gap-1"
                    >
                      {amenityIconMap[a]}
                      <span className="capitalize">{a.replace("-", " ")}</span>
                    </span>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Pay Rent {openPayFor ? `· ${openPayFor.title}` : ""}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={confirmPayment}>
                <IndianRupee className="h-4 w-4 mr-2" /> Pay now
              </Button>
            </div>
          </div>
        </DialogContent>
